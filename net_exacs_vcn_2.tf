...
locals {
  ...
  exa_vcn_2 = local.add_exa_vcn2 == true ? {
    "EXA-VCN-2" = {
      ...
      security                         = local.enable_zpr == true ? {zpr_attributes = [{namespace:"${local.zpr_namespace_name}",attr_name:"net", attr_value:"exa-vcn-2-${local.zpr_label}"}]} : null
            ...
            { for cidr in var.hub_vcn_cidrs : "INGRESS-FROM-SSH-${cidr}-RULE" =>
              {
                ...
                dst_port_min = 22
                dst_port_max = 22
              } if local.hub_with_vcn == true && var.exa_vcn2_attach_to_drg == true && local.add_exa_vcn2 == true
            },
            {
