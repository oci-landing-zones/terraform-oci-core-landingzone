...
locals {
  ...
  oke_vcn1_external_allowed_cidrs_to_ports_into_services_tier = local.add_oke_vcn1 == true ? flatten([for cidr in var.oke_vcn1_external_allowed_cidrs_into_services_tier : [for port in var.oke_vcn1_services_ingress_destination_ports : "${trimspace(cidr)},${trimspace(port)}" ]]) : []
  ...
  oke_vcn_1_to_services_subnet_cross_vcn_ingress = merge(
    ...
    (local.hub_with_vcn == true || local.hub_with_drg_only == true) ? { for cidr_port_pair in flatten([for cidr in var.allowed_onprem_cidrs_to_app_endpoints : [for port in var.oke_vcn1_services_ingress_destination_ports : "${trimspace(cidr)},${trimspace(port)}" ]]) : "INGRESS-FROM-${split(",",cidr_port_pair)[0]}-ON-${split(",",cidr_port_pair)[1]}-RULE" => {
