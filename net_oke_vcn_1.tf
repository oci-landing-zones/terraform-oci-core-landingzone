...
locals {
  ...
  oke_vcn_1_to_services_subnet_cross_vcn_egress = merge(
    ...
    (local.hub_with_vcn == true || (local.hub_with_drg_only == true && (length(var.oke_vcn1_routable_vcns) == 0 || contains(var.oke_vcn1_routable_vcns, "OKE-VCN-3")))) ? { for port in var.oke_vcn3_services_ingress_destination_ports : "EGRESS-TO-OKE-VCN3-ON-${port}-RULE" => {
      description  = "Egress to ${local.oke_vcn3_services_subnet_display_name}."
      stateless    = false
      protocol     = "TCP"
  ...
  oke_vcn_1_to_workers_subnet_cross_vcn_ingress = merge(
    ...
    (local.hub_with_vcn == true || (local.hub_with_drg_only == true && (length(var.oke_vcn1_routable_vcns) == 0 || contains(var.oke_vcn1_routable_vcns, "OKE-VCN-3")))) ? {
      ...
      "INGRESS-FROM-OKE-VCN-3-SERVICES-SUBNET-RULE" = {
        description  = "Ingress from ${local.oke_vcn3_services_subnet_display_name}."
        stateless    = false
        protocol     = "TCP"
        src          = local.oke_vcn3_services_subnet_cidr
        src_type     = "CIDR_BLOCK"
        dst_port_min = 30000
        dst_port_max = 32767
      },
