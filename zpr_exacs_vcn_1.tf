locals {
  ...
  exa_1_hub_zpr_grants = ((local.add_exa_vcn1) && (local.hub_with_vcn == true) && (var.exa_vcn1_attach_to_drg == true) && (var.deploy_bastion_jump_host)) ? [
    "in ${local.zpr_namespace_name}.net:exa-vcn-1 VCN allow '${coalesce(var.hub_vcn_jumphost_subnet_cidr, cidrsubnet(var.hub_vcn_cidrs[0], 3, 4))}' to connect to ${local.zpr_namespace_name}.database:${local.zpr_label} endpoints with protocol='tcp/22'"
  ] : []
  ...
  exa_1_zpr_policy = local.add_exa_vcn1 ? {
    ZPR-POLICY-EXA-1 = {
      description = "${var.lz_provenant_label} ZPR policy for ${var.service_label}-${coalesce(var.exa_vcn1_name, "exa-vcn-1")}-zpr-policy."
      name        = "${var.service_label}-${coalesce(var.exa_vcn1_name, "exa-vcn-1")}-zpr-policy"
