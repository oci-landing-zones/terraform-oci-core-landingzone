...
This template shows how to deploy a CIS compliant landing zone using [OCI Core Landing Zone](../../) configured with a Hub & Spoke networking topology using a FastConnect Virtual Circuit for on-premises connectivity. It deploys a FastConnect Partner conection using the partner provider ocid, a Three Tier VCN, and an Exadata VCN which are peered through the DRG. The DRG is configured to route traffic across all VCNs. To get a list of the available service offerings, see [ListFastConnectProviderServices](https://docs.cloud.oracle.com/iaas/api/#/en/iaas/latest/FastConnectProviderService/ListFastConnectProviderServices).
...
| fastconnect\_virtual\_circuit\_customer\_bgp\_peering\_ip | The BGP IPv4 address for the edge router on the other end of the BGP session from Oracle. Must use a subnet mask from /28 to /31. | "x.x.x.x/x"
| fastconnect\_virtual\_circuit\_oracle\_bgp\_peering\_ip | The IPv4 address for Oracles end of the BGP session. Must use a subnet mask from /28 to /31. | "x.x.x.x/x"
| fastconnect\_virtual\_circuit\_provider\_service\_id | The OCID of the service offered by the provider (if you are connecting via a provider). | "ocid1.providerservice.oc1..."
