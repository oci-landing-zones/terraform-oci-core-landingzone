locals {
  ...
  hub_on_prem_zpr_grants = local.hub_with_vcn && var.deploy_bastion_jump_host ? [
    for cidr in local.all_onprem_cidrs :
  ...
  hub_zpr_policy = local.hub_with_vcn && var.deploy_bastion_jump_host ? {
    ZPR-POLICY-HUB-VCN = {
      description = "${var.lz_provenant_label} ZPR policy for ${var.service_label}-${coalesce(var.hub_vcn_name, "hub-vcn")}-zpr-policy."
