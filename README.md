...
1. [Architecture](#arch)
    ...
    1. [Mapping to CIS OCI Benchmark v3.0.0](./ARCH-MAPPING-CIS.md)
...
The OCI Core Landing Zone deploys a standardized environment in an Oracle Cloud Infrastructure (OCI) tenancy that is compliant with the [CIS OCI Foundations Benchmark v3.0](https://www.cisecurity.org/benchmark/oracle_cloud/).
...
6. Ability to deploy OCI Bastion service and a Jump host in the Hub VCN.
...
The OCI Core Landing Zone creates specific compartments in the tenancy root compartment or under an enclosing compartment:
 ...
 - Exadata infrastructure compartment: this optional compartment separates Exadata Cloud Service and Exadata Cloud@Customer infrastructure administration from database administration. When it is not deployed, database administrators manage both Exadata infrastructure and databases in the Database compartment.
...
#### <a name="three-tier">Three Tier VCNs</a>

The three-tier VCNs are comprised of:

 - One (public or private) subnet for load balancers.
...
#### <a name="exacs">Exadata VCNs</a>

The Exadata VCNs are comprised of:

- One private client subnet.
- One optional private backup subnet.
- One optional private integration subnet.

#### <a name="oke">OKE VCNs</a>

The OKE VCNs are comprised of:

- One (public or private) subnet for load balancers.
- Two private subnets: one for worker nodes, one for API endpoint.
- One optional private subnet for Kubernetes pods deployment, supporting OCI native CNI (Container Networking Interface).
- One optional private subnet for cluster management.
- One optional private subnet for database services.

#### <a name="hub">Hub VCN</a>
...
- One optional private subnet for jump host and OCI Bastion service deployment.
...
The diagram below shows services and resources that are deployed along with standalone VCN deployments:

![arch_simple](./images/arch_simple.png)

[Get the diagram in SVG format. SVGs can be read by drawio tool](./images/arch_simple.svg)

The diagram below expands the resources in the network compartment using a third party firewall (Palo Alto Networks or Fortinet):

![arch-advanced-net-appliance](./images/arch-advanced-net-appliance.png)

[Get the diagram in SVG format. SVGs can be read by drawio tool](./images/arch-advanced-net-appliance.svg)

The diagram below expands the resources in the network compartment using OCI Network Firewall:

![arch-advanced-oci-firewall](./images/arch-advanced-oci-firewall.png)

[Get the diagram in SVG format. SVGs can be read by drawio tool](./images/arch-advanced-oci-firewall.svg)
...
A non-exhaustive list of deployment scenarios is available under the [templates](./templates/) folder:
