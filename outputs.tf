...
output "nlb_private_ip_addresses" {
    value = var.display_output && length(module.lz_nlb) > 0 ? merge({for k, v in module.lz_nlb[0].nlbs_private_ips : k => {id:v.id}},{for k, v in module.lz_nlb[0].nlbs_public_ips : k => {id:v.private_ip_id}}) : null
}