  ...
  instances_configuration = local.chosen_firewall_option != "NO" && local.chosen_firewall_option != "OCINFW" ? {
    ...
    instances = {
      FW-1 = {
        ...
        encryption = {
          kms_key_id                            = var.cis_level == "2" ? module.lz_vault.keys["NFW-KEY"].id : null
...
module "lz_nlb" {
  count             = local.chosen_firewall_option != "NO" && local.chosen_firewall_option != "OCINFW" ? 1 : 0
  source            = "github.com/oci-landing-zones/terraform-oci-modules-networking//modules/nlb?ref=v0.7.5"
