...
locals {
  ...
  enable_database_admin_persona      = local.enable_database_compartment || local.enable_exainfra_compartment

  database_admin_group = length(var.existing_database_admin_group_name) == 0 && length(trimspace(var.rm_existing_database_admin_group_name)) == 0 && local.enable_database_admin_persona ? {
  ...
  custom_domain_database_admin_group = var.deploy_custom_domain_groups && local.enable_database_admin_persona ? {
    (local.database_admin_group_key) = {
      identity_domain_id = trimspace(var.custom_id_domain_ocid)
      name               = local.provided_database_admin_group_name
      description        = "${var.lz_provenant_label} group for managing databases."
  ...
  exainfra_admin_group = length(var.existing_exainfra_admin_group_name) == 0 && length(trimspace(var.rm_existing_exainfra_admin_group_name)) == 0 && local.enable_exainfra_compartment ? {
    (local.exainfra_admin_group_key) = {
      ...
      description   = "${var.lz_provenant_label} group for managing Exadata Cloud Service and Exadata Cloud@Customer infrastructure."
      members       = []
      defined_tags  = local.groups_defined_tags
      freeform_tags = local.groups_freeform_tags
    }
