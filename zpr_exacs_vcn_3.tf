locals {
  ...
  exa_vcn_3_zpr_policies = merge(
    ...
    var.add_exa_vcn3 && var.add_exa_vcn1 ? {
      ZPR-POLICY-EXA-VCN-3-TO-EXA-VCN-1 = {
        description = "zpr-policy-exa-vcn-3-to-exa-vcn-1"
        name        = "zpr-policy-exa-vcn-3-to-exa-vcn-1"
        statements = [
    ...
    var.add_exa_vcn3 && var.add_exa_vcn2 ? {
      ZPR-POLICY-EXA-VCN-3-TO-EXA-VCN-2 = {
        description = "zpr-policy-exa-vcn-3-to-exa-vcn-2"
        name        = "zpr-policy-exa-vcn-3-to-exa-vcn-2"
        statements = [
          // Allow endpoints in Client subnet of Exadata VCN to communicate with other Exadata client subnets in other VCNs using SQLNet
    ...
    var.add_exa_vcn3 && var.add_tt_vcn1 ? {
      ZPR-POLICY-EXA-VCN-3-TO-TT-VCN-1 = {
        description = "zpr policy for exa vcn 3 to tt vcn 1"
        name        = "zpr-policy-exa-vcn-3-to-tt-vcn-1"
        statements = [
          ...
          "in ${var.zpr_security_attributes_namespace}.net:exa-vcn-3-${local.zpr_label} VCN allow ${coalesce(var.tt_vcn1_db_subnet_cidr, cidrsubnet(var.tt_vcn1_cidrs[0], 4, 2))} endpoints to connect to ${var.zpr_security_attributes_namespace}.database:${local.zpr_label} with protocol='tcp/1521-1522'"
        ]
      }
    } : {},
    var.add_exa_vcn3 && var.add_tt_vcn2 ? {
      ZPR-POLICY-EXA-VCN-3-TO-TT-VCN-2 = {
        description = "zpr policy for exa vcn 3 to tt vcn 2"
        name        = "zpr-policy-exa-vcn-3-to-tt-vcn-2"
        statements = [
          // Allow DB Server to communicate with other endpoints in the TT VCNs
          ...
          // Allow DB Subnet endpoints in Three Tier VCNs connect to DB Subnet in Exadata / Ingress
          "in ${var.zpr_security_attributes_namespace}.net:exa-vcn-3-${local.zpr_label} VCN allow ${coalesce(var.tt_vcn2_db_subnet_cidr, cidrsubnet(var.tt_vcn2_cidrs[0], 4, 2))} endpoints to connect to ${var.zpr_security_attributes_namespace}.database:${local.zpr_label} with protocol='tcp/1521-1522'"
        ]
      }
    } : {},
    var.add_exa_vcn3 && var.add_tt_vcn3 ? {
      ZPR-POLICY-EXA-VCN-3-TO-TT-VCN-3 = {
        description = "zpr policy for exa vcn 3 to tt vcn 3"
        name        = "zpr-policy-exa-vcn-3-to-tt-vcn-3"
        statements = [
          // Allow DB Server to communicate with other endpoints in the TT VCNs
          ...
          // Allow DB Subnet endpoints in Three Tier VCNs connect to DB Subnet in Exadata / Ingress
          "in ${var.zpr_security_attributes_namespace}.net:exa-vcn-3-${local.zpr_label} VCN allow ${coalesce(var.tt_vcn3_db_subnet_cidr, cidrsubnet(var.tt_vcn3_cidrs[0], 4, 2))} endpoints to connect to ${var.zpr_security_attributes_namespace}.database:${local.zpr_label} with protocol='tcp/1521-1522'"
        ]
      }
    } : {}
  )
