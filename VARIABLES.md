...
| enable\_cross\_vcn\_constrained\_nsgs | When true, Landing Zone provisions constrained NSGs that enable DRG-attached and routable VCNs to connect according to Landing Zone provided rules. Enabled by default and takes precedence over enable\_cross\_vcn\_open\_nsg for spoke VCNs when both are true. | bool | true | no |
| enable\_cross\_vcn\_open\_nsg | When true, Landing Zone provisions an open NSG that enables DRG-attached and routable VCNs to fully connect. For spoke VCNs, it is effective only when enable\_cross\_vcn\_constrained\_nsgs is false. For Hub VCN, the open NSG is created when requested and left unattached by default. | bool | false | no |
