locals {
  ...
  tt_1_zpr_policy = local.add_tt_vcn1 ? {
    ZPR-POLICY-TT-1 = {
      description = "Core Landing Zone ZPR policy for ${coalesce("${var.service_label}-${var.tt_vcn1_name}", "${var.service_label}-tt-vcn-1")}."
      name        = "${coalesce("${var.service_label}-${var.tt_vcn1_name}", "${var.service_label}-tt-vcn-1")}-zpr-policy"
