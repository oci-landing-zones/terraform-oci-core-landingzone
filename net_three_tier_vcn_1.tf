...
locals {
  ...
  tt_vcn1_private_endpoint_subnet_display_name = coalesce(var.tt_vcn1_private_endpoint_subnet_name, "${var.service_label}-three-tier-vcn-1-private-endpoint-subnet")
  tt_vcn1_private_endpoint_subnet_dns_label    = substr(replace(coalesce(var.tt_vcn1_private_endpoint_subnet_name, "pe-subnet"), "/[^\\w]/", ""), 0, 14)
  tt_vcn1_private_endpoint_subnet_cidr         = coalesce(var.tt_vcn1_private_endpoint_subnet_cidr, try(cidrsubnet(var.tt_vcn1_cidrs[0], 28 - tonumber(split("/", var.tt_vcn1_cidrs[0])[1]), pow(2, 28 - tonumber(split("/", var.tt_vcn1_cidrs[0])[1])) - 1), null))
  ...
  tt_vcn_1 = local.add_tt_vcn1 == true ? {
    "TT-VCN-1" = {
      ...
      subnets = merge(
        ...
        var.add_tt_vcn1_private_endpoint_subnet == true ? {
          "TT-VCN-1-PRIVATE-ENDPOINT-SUBNET" = {
            cidr_block                 = local.tt_vcn1_private_endpoint_subnet_cidr
            dhcp_options_key           = "default_dhcp_options"
            display_name               = local.tt_vcn1_private_endpoint_subnet_display_name
            dns_label                  = local.tt_vcn1_private_endpoint_subnet_dns_label
            ipv6cidr_blocks            = []
            prohibit_internet_ingress  = true
            prohibit_public_ip_on_vnic = true
            route_table_key            = "TT-VCN-1-PRIVATE-ENDPOINT-ROUTE-TABLE"
            security_list_keys         = local.tt_vcn1_private_endpoint_subnet_security_list != null ? ["TT-VCN-1-PRIVATE-ENDPOINT-SL"] : []
          }
        } : {}
      ) # merge function
      ...
      security_lists = merge(
        ...
        var.add_tt_vcn1_private_endpoint_subnet && local.tt_vcn1_private_endpoint_subnet_security_list != null ? {
          "TT-VCN-1-PRIVATE-ENDPOINT-SL" = local.tt_vcn1_private_endpoint_subnet_security_list
        } : {}
      )
      ...
      route_tables = merge(
        ...
        var.add_tt_vcn1_private_endpoint_subnet == true ? {
          "TT-VCN-1-PRIVATE-ENDPOINT-ROUTE-TABLE" = {
            display_name = "private-endpoint-route-table"
            route_rules = merge(
              local.tt_vcn1_enable_intra_vcn_drg_route && var.tt_vcn1_attach_to_drg ? {
                "APP-SUBNET-RULE" = {
                  network_entity_key = "HUB-DRG"
              ...
              local.tt_vcn1_private_endpoint_subnet_additional_route_rules
            )
          }
        } : {}
      ) # merge function
      ...
      network_security_groups = merge(
        ...
        var.enable_generative_ai_infra == true ? {
          "TT-VCN-1-GEN-AI-NSG" = {
            ...
            ingress_rules = var.add_tt_vcn1_private_endpoint_subnet ? {
            ...
            egress_rules = var.add_tt_vcn1_private_endpoint_subnet ? {} : {
              "EGRESS-TO-OSN-HTTPS" = {
        ...
        var.enable_aidp_infra == true ? {
          "TT-VCN-1-AIDP-NSG" = {
            ...
            ingress_rules = merge(
              var.add_tt_vcn1_private_endpoint_subnet ? merge(
                {
              ...
              local.tt_vcn1_aidp_nsg_additional_ingress_rules
            )
            egress_rules = merge(
              var.add_tt_vcn1_private_endpoint_subnet ? {} : {
                "EGRESS-TO-OSN-HTTPS" = {
              ...
              local.tt_vcn1_aidp_nsg_additional_egress_rules
            )
          }
        } : {},
