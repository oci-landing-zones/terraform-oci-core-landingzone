locals {
  ...
  exa_2_hub_zpr_grants = ((local.add_exa_vcn2) && (local.hub_with_vcn == true) && (var.exa_vcn2_attach_to_drg == true)) ? [
    ...
    "in ${local.zpr_namespace_name}.net:exa-vcn-2 VCN allow '${cidr}' to connect to ${local.zpr_namespace_name}.database:${local.zpr_label} endpoints with protocol='tcp/22'"
  ] : []
