locals {
  ...
  tt_3_hub_zpr_grants = local.add_tt_vcn3 && local.hub_with_vcn && var.tt_vcn3_attach_to_drg ? [
    "in ${local.zpr_namespace_name}.net:tt-vcn-3 VCN allow '${coalesce(var.hub_vcn_mgmt_subnet_cidr, cidrsubnet(var.hub_vcn_cidrs[0], 2, 3))}' to connect to ${local.zpr_namespace_name}.database:${local.zpr_label} endpoints with protocol='tcp/22'"
  ] : []
