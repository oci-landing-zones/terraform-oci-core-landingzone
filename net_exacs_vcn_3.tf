...
locals {
            ...
            { for cidr in var.hub_vcn_cidrs : "INGRESS-FROM-SSH-${cidr}-RULE" =>
              {
                ...
                dst_port_min = 22
                dst_port_max = 22
              } if local.hub_with_vcn == true && var.exa_vcn3_attach_to_drg == true && local.add_exa_vcn3 == true
            },
            {
  ...
  exa_vcn_3_drg_routing = merge(
    (local.add_exa_vcn3 == true && var.exa_vcn3_attach_to_drg == true && local.chosen_hub_option != 0) ? { 
      "DRG-RULE" = {
        network_entity_key = "HUB-DRG"
        description        = "To DRG."
        destination        = "0.0.0.0/0"
